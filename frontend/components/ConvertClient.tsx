...
export default function ConvertClient() {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (data.progress > 0 && data.progress < 100) {
          ...
          // If no progress for 3 minutes, show warning
        ...
        // If too many failures, assume backend is down
        if (failureCount >= maxFailures) {
          clearInterval(interval);
          setJobStatus("failed");
          setStatusMessage(
            "Backend service is not responding. The server may be sleeping (free tier) " +
            "or experiencing issues. Please try again in a few minutes."
          );
        }
      }
  ...
  const getFriendlyMessage = () => {
    if (stuckDetected) {
      return "⚠️ Processing is taking longer than expected. This may be due to server limitations on the free tier.";
    }
            ...
            <div className="p-8 space-y-6">
              <div className="text-center">
                ...
                <p className="text-xs text-yellow-500/80 mt-2">
                  ⚠️ Free tier limit: 50 pages per PDF
                </p>
              </div>
          ...
          <div className="mx-auto max-w-2xl rounded-2xl bg-white/5 backdrop-blur-md
            ...
            <div className="text-center space-y-6">
              ...
              <div>
                ...
                {statusMessage.includes("free tier") && (
                  <div className="mt-4 p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg">
                    <p className="text-xs text-blue-400">
                      💡 Free tier has limited resources. Try:
                      <br />• Smaller PDF (under 20 pages)
                      <br />• Wait a few minutes and retry
                      <br />• Use text-only PDFs instead of scanned ones
                    </p>
                  </div>
                )}
              </div>
