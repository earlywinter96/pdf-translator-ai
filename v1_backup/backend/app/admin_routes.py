...
# ============================================================================
# OPENAI MONITORING ENDPOINTS (ADD THESE)
# ============================================================================

@admin_router.get("/admin/openai/status")
async def get_openai_status(x_admin_auth: str = Header(None)):
    """Get OpenAI integration status and budget info"""
    verify_admin_auth(x_admin_auth)
    
    try:
        import os
        from app.utils.usage_tracker import get_usage_stats, load_usage
        ...
        if mock_mode:
            return {
                "mode": "mock",
                "status": "Mock mode enabled - no real API calls",
                "api_key_configured": False,
            }
        
        # Production mode
        from app.utils.openai_wrapper import get_openai_client
        ...
        try:
            ...
            monthly_stats = usage_data.get("monthly_stats", {}).get(current_month, {
                "cost_inr": 0.0,
                "requests": 0,
                "tokens": 0
            })
            ...
            return {
                ...
                "model": client.model,
                "budget": {
                    "limit_inr": budget_status["budget_limit_inr"],
                    "current_usage_inr": budget_status["current_usage_inr"],
                    "remaining_inr": budget_status["remaining_inr"],
        ...
        logger.error(f"Error getting OpenAI status: {e}")
        raise HTTPException(500, f"Failed to get OpenAI status: {str(e)}")
